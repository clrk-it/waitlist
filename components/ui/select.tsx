...
function SelectTrigger({
  ...
  return (
    <Trigger
      ...
      data-size={size}
      className={cn(
        "flex w-full items-center justify-between gap-2 rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-foreground shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
        "data-placeholder:text-muted-foreground",
        // Glass look inside the dark marketing shell.
    ...
    <Portal>
      <Content
        ...
        className={cn(
          ...
          // Dark marketing look: slightly opaque black panel + subtle border.
...
function SelectItem({
  ...
  return (
    <Item
      ...
      className={cn(
        "focus:bg-primary focus:text-primary-foreground relative flex w-full cursor-default items-center gap-2 rounded-md py-2 pr-8 pl-3 text-sm text-foreground outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
