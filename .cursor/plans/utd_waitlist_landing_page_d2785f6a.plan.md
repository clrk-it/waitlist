---
name: UTD Waitlist Landing Page
overview: Build a modern dark-mode waitlist landing page for UTD Club Store with glassmorphism effects, mesh gradient background, and responsive design using Next.js, Tailwind CSS, and shadcn/ui components.
todos:
  - id: install_shadcn
    content: Install shadcn/ui Button and Input components using CLI
    status: completed
  - id: update_css_vars
    content: Update globals.css with UTD brand colors in HSL format (primary orange, secondary green, dark background)
    status: completed
  - id: update_layout
    content: Add dark class to html element and update metadata in layout.tsx
    status: completed
  - id: create_background
    content: Create mesh gradient background with pseudo-elements using UTD brand colors
    status: completed
    dependencies:
      - update_css_vars
  - id: create_navbar
    content: Add centered UTD logo/navbar at the top of the page
    status: completed
  - id: create_hero_card
    content: Build glassmorphism card with headline, subtext, email input, and button
    status: completed
    dependencies:
      - install_shadcn
      - update_css_vars
  - id: create_social_proof
    content: Add avatar stack and social proof text below the action area
    status: completed
  - id: make_responsive
    content: Ensure mobile stacking and desktop side-by-side layout for input/button
    status: completed
    dependencies:
      - create_hero_card
---
